...
public class AuctionHouseImp implements AuctionHouse {
    ...
    public List<CatalogueEntry> viewCatalogue() {
        ...
        Integer[] lotNums = lots.keySet().toArray(new Integer[lots.size()]);
    ...
    public Status noteInterest(
        ...
        if (buyers.containsKey(buyerName)) {
            ...
            if(lots.containsKey(lotNumber)) {
                //check if buyer is already registered inside the addInterestedBuyer()
    ...
    public Status openAuction(
        ...
        if (currentLot.getStatus() == LotStatus.UNSOLD) {
          //sends a message to interested buyers and the seller
           String selleraddress = sellers.get(currentLot.getSellerName()).getAddress();
           ...
           if(auctioneers.containsKey(auctioneerName)) {
               ...
               if (!auctioneers.get(auctioneerName).inAuction()) {
                   ...
                   return Status.error("Auctioneer already in an other auction");
               }
               
           }//creates new auctioneer if not in database
           else {
               auctioneers.put(auctioneerName,new Auctioneer(auctioneerName,auctioneerAddress));
           }
           //open auction
           auctions.put(lotNumber, new Auction(lots.get(lotNumber),auctioneers.get(auctioneerName)));
           return Status.OK();
        }
              ...
              //lot in auction
                if(lots.get(lotNumber).getStatus()== LotStatus.IN_AUCTION) {
                  //bid is OK to go through
                    Money difference = bid.subtract(auctions.get(lotNumber).getCurrentBid());
                    //difference has to be >= increment
                    if (difference.compareTo(parameters.increment)>=0) {
                        //placing bid
                        auctions.get(lotNumber).setCurrentBid(bid);
                        auctions.get(lotNumber).setHighestBidder(buyers.get(buyerName));
                        
                        //sending messages to
                        //seller,auctioneer
                        String selleraddress = sellers.get(currentLot.getSellerName()).getAddress();
                        msg.bidAccepted(selleraddress, lotNumber, bid);
                        String auctaddress = auctions.get(lotNumber).getAuctioneerAddress();
                        ...
                        return Status.error("Too Small bid,this bid has to be >= ("
                        +parameters.increment+"+"+auctions.get(lotNumber).getCurrentBid());
                    }
                }else {
                ...
                return Status.error("Lot not in database, so its not in auction so cant make bid");
            }
        }else {
            logger.warning(startBanner("Buyer not in database"));
            return Status.error("Buyer not in database, so he/she cant make a bid");
        }
            
    }
    private Status gatherPayment(int lotNumber,Money highest_bid,Buyer highestBidder) {
        ...
        return bank.transfer(senderAccount, senderAuthCode, receiverAccount, amount);
        
    }
    private Status makePayment(int lotNumber,Money highest_bid,Buyer highestBidder) {
        //sellers.get(lots.get(lotNumber).getSellerName()).getAddress();
          ...
          String receiverAccount = sellers.get(lots.get(lotNumber).getSellerName()).getBankAccount();
                      ...
                      if (highest_bid!=new Money("0")) {
                        ...
                        //highest bid is bigger than reserve price
                          if (highest_bid.compareTo(resP)>=0) {
                              //close auction
                              auctions.get(lotNumber).setLotStatus(LotStatus.SOLD_PENDING_PAYMENT);
                              auctioneers.get(auctioneerName).setStatus(false);//auctioneer's job ended
                              
                              //payment
                              Buyer highestBidder = auctions.get(lotNumber).getHighestBidder();
                              Status in = gatherPayment(lotNumber,highest_bid,highestBidder);
                              //Stage 1 ok
                              if (Status.Kind.OK ==in.kind) {
                                  Status out = makePayment(lotNumber,highest_bid,highestBidder);
                                  //Stage 2 OK
                                  if (Status.Kind.OK ==out.kind) {
                                      auctions.get(lotNumber).setLotStatus(LotStatus.SOLD);
                                      //messages
                                      //seller
                                      String selleraddress = sellers.get(currentLot.getSellerName()).getAddress();
                                      ...
                                      return new Status(Status.Kind.SALE, "Sale went through");
                                  }//didnt go through Stage 2
                                  else {
                                      logger.warning(startBanner(out.message));
                                      return new Status(Status.Kind.SALE_PENDING_PAYMENT, out.message);
                                  }
                              }//didnt go through Stage 1
                              else {
                                  logger.warning(startBanner(in.message));
                                  return new Status(Status.Kind.SALE_PENDING_PAYMENT, in.message);
                              }
                          }
                          else {
                              logger.fine(startBanner("Sale didnt go through"));
                              auctions.get(lotNumber).setLotStatus(LotStatus.UNSOLD);
                            ...
                            //messages
                              ...
                              return new Status(Status.Kind.NO_SALE, "Sale didnt go through");
                          }
                      }else{
                          logger.warning(startBanner("Lot Not in auction"));
                          return new Status(Status.Kind.NO_SALE, "Sale didnt go through");
