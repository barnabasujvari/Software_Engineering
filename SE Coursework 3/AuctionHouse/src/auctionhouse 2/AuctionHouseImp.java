...
public class AuctionHouseImp implements AuctionHouse {
    ...
    public static void main(String[] args) {
        ...
        final double BUYER_PREMIUM = 10.0;
        final double COMMISSION = 15.0;
        final Money INCREMENT = new Money("10.00");
        final String HOUSE_ACCOUNT = "AH A/C";
        final String HOUSE_AUTH_CODE = "AH-auth";
        ...
        // Ensure the root handler passes on all messages at loggingLevel and
        // above (i.e. more severe)
        ...
        house = new AuctionHouseImp(new Parameters(BUYER_PREMIUM, COMMISSION,
                INCREMENT, HOUSE_ACCOUNT, HOUSE_AUTH_CODE, messagingService,
                bankingService));

        /*
         * Setup story running through all the test cases.
         * 
         * Story end point is made controllable so that tests can check story
         * prefixes and branch off in different ways.
         */
        house.registerSeller("SellerY", "@SellerY", "SY A/C");
        house.registerSeller("SellerZ", "@SellerZ", "SZ A/C");

        house.addLot("SellerY", 2, "Painting", new Money("200.00"));
        house.addLot("SellerY", 1, "Bicycle", new Money("80.00"));
        house.addLot("SellerZ", 5, "Table", new Money("100.00"));

        house.registerBuyer("BuyerA", "@BuyerA", "BA A/C", "BA-auth");
        house.registerBuyer("BuyerB", "@BuyerB", "BB A/C", "BB-auth");
        house.registerBuyer("BuyerC", "@BuyerC", "BC A/C", "BC-auth");

        house.noteInterest("BuyerA", 1);
        house.noteInterest("BuyerA", 5);
        house.noteInterest("BuyerB", 1);
        house.noteInterest("BuyerB", 2);

        house.openAuction("Auctioneer1", "@Auctioneer1", 1);
        ...
        house.closeAuction("Auctioneer1", 1);
        messagingService.expectLotSold("@BuyerA", 1);
        messagingService.expectLotSold("@BuyerB", 1);
        messagingService.expectLotSold("@SellerY", 1);
        messagingService.verify();

        bankingService.expectTransfer("BB A/C", "BB-auth", "AH A/C",
                new Money("110.00"));
        bankingService.expectTransfer("AH A/C", "AH-auth", "SY A/C",
                new Money("85.00"));
        bankingService.verify();

        List<CatalogueEntry> expectedCatalogue = new ArrayList<CatalogueEntry>();
        expectedCatalogue
                .add(new CatalogueEntry(1, "Bicycle", LotStatus.UNSOLD));
        expectedCatalogue
                .add(new CatalogueEntry(2, "Painting", LotStatus.UNSOLD));
